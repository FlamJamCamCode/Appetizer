/* ============================================
   NAV-VIMLIKE - MODULE LOADER
   Professional modular vim navigation system
   
   Include *only this file* to enable the full
   vim-like navigation stack. It will automatically
   load shared-lobby-tabs.js (if needed), inject the
   base stylesheets, and register every behaviour.
   ============================================ */

(function() {
    'use strict';

    if (window.__navVimlikeLoading) {
        console.warn('VimExt: nav-vimlike.js already requested, skipping duplicate load');
        return;
    }

    if (window.LobbyTabsVim && window.VimModeManager && window.VimModeManager.handleKeyPress) {
        console.warn('VimExt: nav-vimlike.js already initialized');
        return;
    }

    window.__navVimlikeLoading = true;
    ...
    function loadModulesSequentially() {
        ...
        return chain.then(() => {
            ...
            window.__navVimlikeLoading = 'ready';
        }).catch((err) => {
            console.error('VimExt: Module load failure', err);
            window.__navVimlikeLoading = false;
            throw err;
        });
    }
