...
\ Context: This is an example of "fluid-code" projected into use somewhere. Therefore there is no issue in hardcodedness in the code like 0.89 (it is subject to be changed; hence also languages which are basically simply synonyms of the opcode level of the actuation machine; all optimization is in the code generation; strictly superior optimization possible this way; And by blob-class + encoding scheme, you can have words that are not at all code comprehensible for humans become nicely wrapped words anyway) or the words being particular and opinionated about reality below and beyond its real confines
