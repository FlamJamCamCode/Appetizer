...
- Enables Christian, Islamic, Anarchist, Communist, Fascist zones equally (Attempted ubiquity morality see MBC; Allows for retrying as much as one wishes without too permanent taxing. Which is useful for discovery. In all discord, dispute and polarity... Neverending trying another thing again.)
- Each implements their own moral architecture
- System provides mechanism, not values

**Recursive Self-Application**:
Every mechanism applies to itself:
- Architecture describes architecture that actualizes (and discovers in the process) logic that actualizes machine that is actualizing to find architecture
